...
\begin{enumerate}
    ...
    \item Raw data transformation into a data set that to the target schema illustrated in \autoref{fig:overview}.
...
\\
For later analysis, the data must be converted to required target schema, illustrated in \autoref{fig:overview}.
Therefore we employ the model pipeline, which generates the final data set with the attributes future statement, sentiment, topic and url.
...
Finally, bigrams that occurred in more than 60\% statements and less than 20 of the documents were filtered out. The remaining bigrams were inputted into an LDA model, returning clusters of related topics. To label each cluster with a matching headline or cluster name, we used OpenAI’s GPT-3 (text-davinci-002) to turn suggestions of cluster names.
\\
